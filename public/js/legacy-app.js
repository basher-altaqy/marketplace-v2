...
function refreshNav() {
  ...
  navNotificationsBtn?.classList.toggle("hidden", !hasUser);
...
function showView(viewName) {
  ...
  if (viewName === "catalog") catalogView?.classList.remove("hidden");
  if (viewName === "product") productView?.classList.remove("hidden");
  if (viewName === "auth") authView?.classList.remove("hidden");
  if (viewName === "profile") profileView?.classList.remove("hidden");
  if (viewName === "favorites") favoritesView?.classList.remove("hidden");
  if (viewName === "cart") cartView?.classList.remove("hidden");
  if (viewName === "checkout") checkoutView?.classList.remove("hidden");
  if (viewName === "orders") ordersView?.classList.remove("hidden");
  if (viewName === "dashboard") dashboardView?.classList.remove("hidden");
  if (viewName === "seller") sellerView?.classList.remove("hidden");
  if (viewName === "messages") {
    messagesView?.classList.remove("hidden");
    loadConversationsView();
  }
  if (viewName === "notifications") notificationsView?.classList.remove("hidden");
...
async function loadDashboard() {
  ...
  try {
    ...
    if (myProductsGrid) {
      ...
      myProductsGrid.querySelectorAll("[data-my-product-status]").forEach((btn) => {
        btn.addEventListener("click", () => {
          updateMyProductStatus(Number(btn.dataset.myProductStatus), btn.dataset.next);
        });
      });

      bindProductActions(myProductsGrid);
    }
  } catch (error) {
    console.error(error);
  }
